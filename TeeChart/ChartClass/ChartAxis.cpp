...
//
//	int Size = 0;	
//	CSize TickSize = GetLargestTick(pDC);//获取最大刻度高或宽
//	CSize LabelSize = m_pAxisLabel->GetSize(pDC);//坐标轴名称标签大小
//
//	if (m_bIsHorizontal)
//	{
//		//水平轴
//		if (!m_bAutoMargin)
//			Size = m_iMarginSize;//获取指定边缘高度
//		else
//		{
//			Size += 4 + 2;		//Space above and under the text
//
//			Size += TickSize.cy;//刻度值的高度
//			Size += LabelSize.cy;//坐标轴名称的高度
//
//			m_iMarginSize = Size;//边缘的高度等于刻度和坐标轴名称的高度之和
//		}
//
//		if (!m_bIsSecondary)
//		{
//			//bottom轴
//			ControlRect.bottom -= Size;//减去刻度值和坐标轴名称高度
//			ControlRect.right -= TickSize.cx/2+3;//减去刻度值宽度的一半加3
//
...
//		{
//			//top轴
//			ControlRect.top += Size;//减去刻度值和坐标轴名称高度
//			ControlRect.right -= TickSize.cx/2+3;//减去刻度值宽度的一半加3
//
...
int CChartAxis::RefreshMarginSize(CDC* pDC)
{
	...
	if (m_bIsHorizontal)
	{
		...
		m_iVerMarginSize = Size;//边缘的高度等于刻度和坐标轴名称的高度之和

		m_iHorMarginSize = TickSize.cx / 2 + 3;//减去刻度值宽度的一半加3
