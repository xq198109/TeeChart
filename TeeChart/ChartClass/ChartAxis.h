...
public:
	...
	//! Retrieves the position (in %) of the axis.坐标轴位置？
	int GetPosition();
	...
	**/
	void SetInverted(bool bInverted);//设置反向坐标轴
	...
	**/
	void SetAutomatic(bool bAutomatic); //自动设置坐标范围
	...
	**/
	...
	enum EAxisAutoModes 
	{
		//! The axis min and max values are set manually非自动
		NotAutomatic,
		//全自动，根据所有数据确定最小和最大坐标
	...
	//! Sets the automatic mode of the axis设置自动模式
	...
	**/
	...
	//! Retrieves the chart axis label object坐标值
	CChartAxisLabel* GetLabel() const	{ return m_pAxisLabel; }
	//! Retrieves the chart axis grid object坐标格网
	...
	**/
	void SetMarginSize(int iHNewSize,int iVNewSize);
	void GetMarginSize(int& HorMarginSize, int& VerMarginSize) const
	{		 
		HorMarginSize = m_iHorMarginSize;
		VerMarginSize = m_iVerMarginSize;
	};


	void SetAutoMarginSize(bool bEnabled)   { m_bAutoMargin = bEnabled; }
	//! Enable the pan and zoom for this axis.移动和缩放
	...
	**/
	virtual double ScreenToValue(long ScreenVal) const = 0;
...
protected:
	...
	**/
	...
	virtual void RefreshTickIncrement(CDC* pDC) = 0;
	...
	**/
	...
	They do not necessarily belong to the same tick.获取刻度标记的最大宽度或高度
	**/
...
private:
	...
	**/
	...
	/**
		...
		@param PlottingRect
			中心绘图区
	**/
	void SetAxisSize(const CRect& ControlRect, const CRect& PlottingRect);
	...
	**/
	//int ClipMargin(CRect ControlRect, CRect& PlottingRect, CDC* pDC);
	int RefreshMarginSize(CDC* pDC);
	//重新计算刻度间隔和第一个刻度值
	//! RefreshTick the axis properties. 
	/**
		This function simply calls RefreshTickIncrement and 
		RefreshFirstTick.
	**/
	void RefreshTicks(CDC* pDC);
...
//	bool m_bIsAutomatic;  
	...
	//! Specifies if the axis is secondary辅助坐标轴
	/**
...
private:
	...
	int m_iHorMarginSize;
	int m_iVerMarginSize;
